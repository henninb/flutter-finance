...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Dart/Flutter (mobile)**: `dart:io` `Process.run` with user input, insecure `http` package (use `https`), hardcoded secrets or API keys in `pubspec.yaml` or source, missing certificate pinning (`SecurityContext` or a pinning package), `jsonDecode` on untrusted input passed directly to logic without validation, `SharedPreferences` storing sensitive data in plaintext (use `flutter_secure_storage` for secrets — already in use, verify it covers all token storage), `WebView` loading user-controlled URLs without an allowlist, exported `Activity` via `AndroidManifest.xml` without permission checks, keystore credentials hardcoded in `build.gradle.kts` instead of environment variables, debug flags (`kDebugMode`, `EnvConfig.enableLogging`) leaking JWT tokens or API responses in production logs, Dio interceptors logging full request/response bodies including `Authorization` headers in non-development environments
